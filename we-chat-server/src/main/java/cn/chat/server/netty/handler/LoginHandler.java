...
public class LoginHandler extends MyBizHandler<LoginRequest> {
    ...
    public void channelRead(Channel channel, LoginRequest msg) {
        ...
        for (TalkBoxInfo talkBoxInfo : talkBoxInfoList) {
            ...
            // 好友；聊天消息
            if (TalkTypeEnum.Friend.getCode().equals(talkBoxInfo.getTalkType())) {
                List<ChatRecordDto> chatRecordDtoList = new ArrayList<>();
                List<ChatRecordInfo> chatRecordInfoList = userService.queryChatRecordInfoList(talkBoxInfo.getTalkId(), msg.getUserId(), TalkTypeEnum.Friend.getCode());
            ...
            // 群组；聊天消息
            else if (TalkTypeEnum.Group.getCode().equals(talkBoxInfo.getTalkType())) {
                List<ChatRecordDto> chatRecordDtoList = new ArrayList<>();
                List<ChatRecordInfo> chatRecordInfoList = userService.queryChatRecordInfoList(talkBoxInfo.getTalkId(), msg.getUserId(), TalkTypeEnum.Group.getCode());
                for (ChatRecordInfo chatRecordInfo : chatRecordInfoList) {
